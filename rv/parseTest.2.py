...
def rewriteParams(tree, params):
	class RewriteParam(ast.NodeTransformer):
		def visit_arguments(self, node):
			...
			astArgsArray = []
			for param in params:
				astArgsArray.append(ast.Name(id=param, ctx=ast.Param()))
...
def main(): 
	...
	testcasesStr = "operation(\"+\",1,2)///Hello///1"
	...
	if not isFunction(p): 
		...
		try: 
			...
			feedback += "Could not add functionDef due to: " + str(e)
			points = 0
			printResult(points, feedback)
			return 1
	...
	try:
		...
		feedback += "Could not resolve function name due to: " + str(e)
		points = 0
		printResult(points, feedback)
	...
	try:
		...
		feedback += "Could not resolve parameters due to: " + str(e)
		points = 0
		printResult(points, feedback)
	...
	try:  
		...
		feedback += "Could not resolve requirements due to: " + str(e)
		printResult(points, feedback)
